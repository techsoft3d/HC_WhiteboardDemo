        ...
        "src": {
            ...
            "codes.js": function (exports, module, require) {
                ...
                var listTerminator = function (obj) {
                    return util.derive(obj, {
                        eof: true,
                        measure: function (formatting) {
                            return { width: 10, ascent: 0, descent: 0 }; // text.measure(text.enter, formatting);
                        },
            ...
            "doc.js": function (exports, module, require) {
                ...
                var prototype = node.derive({
                    ...
                    byCoordinate: function (x, y) {
                        ...
                        console.log("_frame.actualWidth:" + this.frame._actualWidth);
                        console.log("_frame.height:" + this.frame.height);
                        console.log("_frame.length:" + this.frame.length);
                        console.log("lines.length:" + this.frame.lines.length);
                        for (var i = 0; i < this.frame.lines.length; i++) {
                            console.log("lines.actualwidth:" + this.frame.lines[i].actualWidth);
                            console.log("lines.width:" + this.frame.lines[i].width);
                            console.log("lines.pw0:" + this.frame.lines[i].positionedWords[0].left);
                            console.log("lines.pwl:" + this.frame.lines[i].positionedWords[this.frame.lines[i].positionedWords.length-1].left);
                    ...
                    quickrefresh: function (ordinal, ordinalEnd, takeFocus) {
            ...
            "dom.js": function (exports, module, require) {
                ...
                exports.handleMouseEvent = function (element, name, handler) {
                    exports.handleEvent(element, name, function (ev) {
                        ...
                        return handler(ev, (ev.clientX - rect.left) / carotaScale, (ev.clientY - rect.top) / carotaScale);
                    });
                };
            ...
            "editor.js": function (exports, module, require) {
                ...
                exports.create = function (element) {
                    ...
                    element.innerHTML =
                        '<div id="spacerid" class="carotaSpacer">' +
                    ...
                    function getVerticalOffset() {
                        var docHeight = (doc.frame.bounds().h * carotaScale);
                        if (docHeight < element.clientHeight) {
                            switch (verticalAlignment) {
                                case 'middle':
                                    return (element.clientHeight - docHeight) / 2/carotaScale+15;
                                case 'bottom':
                    ...
                    var paint = function () {
                        ...
                        var scale = parseFloat($(spacer).css("zoom"));
                        scale = 1;
                        canvas.width = dpr * element.clientWidth;
                        ...
                        spacer.style.width = element.clientWidth + 'px';
                        ...
                        ctx.scale(carotaScale, carotaScale);
                    ...
                    var updateTextArea = function () {
                        ...
                        if (endChar) {
                            ...
                            textAreaDiv.style.left = (bounds.l * carotaScale) + 'px';
                            textAreaDiv.style.top = (bounds.t * carotaScale) + 'px';
                            textArea.focus();
                            var scrollDownBy = Math.max(0, (bounds.t + bounds.h)*carotaScale -
                                (element.scrollTop + element.clientHeight));
                            if (scrollDownBy) {
                                element.scrollTop += (bounds.t + bounds.h) * carotaScale -
                                    (element.scrollTop + element.clientHeight);
                            }
                            var scrollUpBy = Math.max(0, element.scrollTop - bounds.t * carotaScale);
                            if (scrollUpBy) {
                                    element.scrollTop -= element.scrollTop - bounds.t * carotaScale;
                            }
                    ...
                    function registerMouseEvent(name, handler) {
                        dom.handleMouseEvent(spacer, name, function (ev, x, y) {
                            handler(doc.byCoordinate(x, y - getVerticalOffset()),ev.button,ev.detail);
                        });
                    }

                    registerMouseEvent('mousedown', function (node, button,detail) {
                        if (button == 2)
                            return;
                        ...
                        if (detail != 3)
                            ...
                            var starto = node.word.line.ordinal;
                            var endo = starto + node.word.line.length;
                            doc.select(starto,endo);
                        }
                    ...
                    registerMouseEvent('mousemove', function (node, button) {
                        if (button == 2)
                            return;
                    ...
                    registerMouseEvent('mouseup', function (node, button) {
                        if (button == 2)
                            return;
                        selectDragStart = null;
                        keyboardX = null;
                ...
                var prototype = node.derive({
                    bounds: function (minimal) {
                        ...
                        return rect(this.left, this.baseline - this.ascent,
                            this.width / carotaScale, this.ascent + this.descent);
                    },
                    parent: function () {
                        return this.doc;
                    },
                        ...
                        if (consumer) {
                            ...
                            if (code && code.block) {
                                ...
                                if (!lineBuffer.length) {
                                    ...
                                    if (carotaItemActive == undefined || !carotaItemActive.autoResizeExtents) {
                                        if (lineWidth + inputWord.text.width > width / carotaScale) {
                                            send(emit);
                                        }
                                    }
                                    store(inputWord, emit);
